...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  const diffMs = new Date(dateEnd) - new Date(dateStart);
...
function getWeekNumberByDate(date) {
  ...
  const dayShift = (inputDate.getDay() + 6) % 7;
  inputDate.setDate(inputDate.getDate() - dayShift + 3);

  const yearStart = new Date(inputDate.getFullYear(), 0, 1);

  return Math.ceil(((inputDate - yearStart) / (1000 * 60 * 60 * 24) + 1) / 7);
}
...
function getNextFridayThe13th(date) {
  const inputDate = new Date(date);
  let result = null;
  if (inputDate.getDate() >= 13 || inputDate.getDate() === 1) {
    inputDate.setMonth(inputDate.getMonth(), 13);
  }

  let currentMonth = inputDate.getMonth();
  do {
    inputDate.setMonth(currentMonth);
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  for (let i = 1; i <= daysTotal; i += 1) {
    const dayOfCycle = i % cycle;
    if (dayOfCycle <= countWorkDays && dayOfCycle !== 0) {
      addDay(currentDate);
    }
